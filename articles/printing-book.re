...
裏表紙のサークル名のフォントはmacOSバンドルの筑紫B丸ゴシックにした。
...
すでにデータは作って入稿した後なので、ここから先の文は書籍には反映されない。

電話して聞くのは次のとおり。
とりあえず日光企画さんの浮間本社に電話する。

 * 振込額はいくらか？
   * 22,740円に送料の加算が必要だと思うが振込総額がわからない。
 * データはあれで大丈夫か？
 ...
 * データは受領している
   * 本日10時に受け取った扱い
   * 順次処理していくのでキューの中で待っとれ
   * データの内容チェックは後で
 * 折り返し電話する
  * 送料など振込総額は未確定なので電話した時に伝えるように申し送りしておく

単に待っておけばいいゾ！といわれたので待つ。
電話かかってくるのやだなー。
 ...
 * 表紙4右下サークル名のフォントが埋込ではないがこのまま印刷してよいか
 * 本文にカラー画像あるけどどうするか 本文フルカラーは入稿期限終わった
   * 別のオプションがあるようだったがそこは求めてないのでよく確認せず
 * 見積もりはメールで送る
   * 後述する表紙のラスタライズ結果確認のデータ送付のついで

これに対して次のようにお願いした。

 * 表紙の出力結果の見本がほしい
   * ラスタライズした結果をメールで送ってもらえることに
 * 本文はそちらでグレースケールに変換してほしい
   * 目次の赤枠は平気だけどURLリンクの水色枠は出力されないかもですよ
   * それは属性情報の表示で印刷では出力されないと思ってました…！
   * じゃあオペレーターさんにデータ送って消して出力できるか聞いてみますね

羊とかに聞いてみたアドバイスのまとめ。

 * てくぶでは目次やリンクの枠線はPDF/X変換過程で出ないように調整する
 * 出力見本（こっちでラスタライズした出力予想データ）を添付するとよい
   * かなり本腰入れたガッツリ対応なので、通常は単にプロに任せたほうが楽

=== 08/03 16:53 もう一回電話する

メールでデータをもらって、内容確認した結果フォントは意図どおり（＝印刷所も同じフォントを所有していた）っぽかったのでそのまま処理をすすめてもらうことにした。

金額の齟齬については僕の料金表の見方が間違っていて、表紙+本文=36Pなので単に100冊36Pところの額を見ればよかったらしい。
というわけでお金を振り込んだ。
勝ったなガハハ！

念のため本文PDFのリンク系の枠線を消す方法を調べておいた@<fn>{nolinks-mod}@<fn>{review-issue808}。

//footnote[nolinks-mod][@<href>{https://github.com/vvakame/C92-Splatoon2Guide/commit/99edc79a5d16e2ebf333e2244daf5aa9000032ea}]
//footnote[review-issue808][@<href>{https://github.com/kmuto/review/issues/808}]

忘れないうちにAffinity Designもお買上げしておいた。
次回はフォントをデータに埋め込みたいのでどうやるのか調べておきたい。

これは、こんどこそ入稿完了するのでは…！？

=== 08/06 13:56 電子版のカードを作成する

TechBoosterでは電子書籍版のURLが記入されているカードを作成して紙版に添えている。
カードは名刺shop.comさん@<fn>{meishishop}に片面フルカラーで表紙絵を刷ってもらう。
今回は僕が入稿でいっぱいいっぱいだったので羊が適当に発注しておいてくれて、今日現物がすでに届いている。
印刷用のシールは@<href>{http://www.a-one.co.jp/product/search/detail.php?id=31511}を買って使っている。
PDFをGoogle Cloud StorageにアップロードしてQRコード作成して印刷データを作成するコードは @atsushieno が作ったらしいので使いたい人は彼をpokeしよう！

//footnote[meishishop][@<href>{https://www.meishishop.com/}]

とりあえずC92電子版の収録はここまで。

シール100枚貼り作業をやって15分22秒かかった。

ちなみに日光企画さんからついぞ電話はこなかったので多分入稿は一発で通ったんだと思う。

=== 08/11 9:00 現物を確認する

前日に会場入りして準備した勢から箱がある報告は聞いていた。
実際に現物を確認するとあー間に合ってるよかったー！という気持ちになる。

現物を見ると表紙の黒のインクの部分が若干盛り上がっていて、逆エンボス加工っぽくなっている。
わーかっこいい加工だねーみたいな反応が一部からあったが、CMYKのK周りの処理が甘かったのでインク特盛り部分が発生してしまっているっぽい。
受領データのノーマライズ的な処理もこちらで一回やったほうがいいのだろう。
どういう処理をすればそのあたりが解決されるかは次回入校時に調べる！

問題の枠線は意図どおり、出力されずに刷り上がってきており一安心。
次回は最初から消して提出しましょう。

=== 08/11 14:23 完売する

100部刷ったが完売してしまった。
130部くらい刷っておけば10冊余るくらいのペースだったのではなかろうか…。
なんだかんだ、TechBoosterブランドに混入してたので完売した感が高い。
ブランド力は正義なのだ…！
